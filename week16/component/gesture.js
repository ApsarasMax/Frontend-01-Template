...
export function enableGesture(element){
	...
	let HOUSE_SYMBOL = Symbol("mouse");

	if(document.ontouchstart !== null){
		element.addEventListener("mousedown", (event)=>{
			contexts[HOUSE_SYMBOL] = Object.create(null);

			start(event, contexts[HOUSE_SYMBOL]);
			let mousemove = event =>{
				move(event, contexts[HOUSE_SYMBOL]);
			};
			let mouseend = event =>{
				end(event, contexts[HOUSE_SYMBOL]);
